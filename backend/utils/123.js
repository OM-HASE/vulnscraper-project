...
async function main() {
  ...
  for (let page = 0; page < MAX_PAGES; page++) {
    ...
    // Track all CVEs from current fetch to later delete obsolete entries
    vulns.forEach((v) => {
    ...
    console.log(`Upserted ${ (result.upsertedCount || 0) + (result.modifiedCount || 0) } records (Total so far: ${totalUpserted}).`);
  ...
  // Optional: Delete old vulnerabilities not found in this scraping round
  // Uncomment below to enable
  /*
  const cveArray = Array.from(allCurrentCVEs);
  const deleteResult = await collection.deleteMany({ cve: { $nin: cveArray } });
